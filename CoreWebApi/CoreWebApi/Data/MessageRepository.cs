            ...
            if (!forSignal)
            {
                ...
                || (UserId.Equals(m.MessageFromUserId) && m.MessageToUserId.Equals(_LoggedIn_UserID))))
                    .Select(o => new GroupMessageForListDto
                    {
                        ...
                        FileName = _context.ChatMessageAttachments.FirstOrDefault(m => m.MessageId == o.Id) != null ? _context.ChatMessageAttachments.FirstOrDefault(m => m.MessageId == o.Id).FileName : "",
                        FileType = _context.ChatMessageAttachments.FirstOrDefault(m => m.MessageId == o.Id) != null ? _context.ChatMessageAttachments.FirstOrDefault(m => m.MessageId == o.Id).FileType : "",
                ...
                for (var i = 0; i < DateTimes.Count(); i++)
                {
                    ...
                    if (dt.Date == Today.Date)
                        ToAdd.TimeToDisplay = Messages.Any(m => m.TimeToDisplay == "Today") ? dt.ToString("hh:mm tt") : "Today";
                    else if (dt.Date == Today.AddDays(-1).Date)
                        ToAdd.TimeToDisplay = Messages.Any(m => m.TimeToDisplay == "Yesterday") ? dt.ToString("hh:mm tt") : "Yesterday";
                    else
